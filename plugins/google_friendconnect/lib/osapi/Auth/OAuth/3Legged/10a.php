 ...
 * @author Chris Chabot
 */
class osapi_Auth_OAuth_3Legged_10a extends osapi_Auth_OAuth_2Legged {
  protected $userId;
   ...
   * @param osapi_Storage $storage storage class to use (file,apc,memcache,mysql)
   * @param osapi_Provider $provider the provider configuration (required to get the oauth endpoints)
   ...
   */
  public function __construct($consumerKey, $consumerSecret, osapi_Storage $storage, osapi_Provider $provider, $localUserId = null, $userId = null) {
    $this->provider = $provider;
    $this->localUserId = $localUserId;
    $this->userId = $userId;
    $this->consumerToken = new OAuth_Consumer($consumerKey, $consumerSecret, NULL);
    $this->signatureMethod = new OAuth_SignatureMethod_HMAC_SHA1();
   ...
   * it uses the osapi_Storage class to do so.
   *
   ...
   * @param any $localUser the *local* user ID (this is not the user's ID on the social network site, but the user id on YOUR site, this is used to link the oauth access token to a local login)
   * @param any $userId the *remote* user ID, you can supply this user id if known but it's completely optional. If set it will be included in the oauth requests in the xoauth_requestor_id field)
   * @return osapi_Auth_OAuth_3Legged the logged-in provider instance
   */
  public static function performOAuthLogin($consumerKey, $consumerSecret, osapi_Storage $storage, osapi_Provider $provider, $localUserId = null, $userId = null) {
    $auth = new osapi_Auth_OAuth_3Legged($consumerKey, $consumerSecret, $storage, $provider, $localUserId, $userId);
  ...
  public function upgradeRequestToken($requestToken, $requestTokenSecret, $oauthVerifier) {
    ...
    if ($ret['http_code'] == '200') {
      ...
      if (!isset($matches['oauth_token']) || !isset($matches['oauth_token_secret'])) {
        throw new osapi_Exception("Error authorizing access key (result was: {$ret['data']})");
      }
      // The token was upgraded to an access token, we can now continue to use it.
      $this->accessToken = new OAuth_Consumer(urldecode($matches['oauth_token']), urldecode($matches['oauth_token_secret']));
      $this->storage->set($this->storageKey, $this->accessToken);
      return $this->accessToken;
    } else {
      throw new osapi_Exception("Error requesting oauth access token, code " . $ret['http_code'] . ", message: " . $ret['data']);
    }
  }
  ...
  protected function requestAccessToken($requestToken, $requestTokenSecret, $oauthVerifier) {
    $accessToken = new OAuth_Consumer($requestToken, $requestTokenSecret);
    $accessRequest = OAuth_Request::from_consumer_and_token($this->consumerToken, $accessToken, "GET", $this->provider->accessTokenUrl, array('oauth_verifier' => $oauthVerifier));
    $accessRequest->sign_request($this->signatureMethod, $this->consumerToken, $accessToken);
    return osapi_IO::send($accessRequest, 'GET', $this->provider->httpProvider);
  }
  ...
  public function obtainRequestToken($callbackUrl, $uid) {
    ...
    if ($ret['http_code'] == '200') {
      ...
      if (!is_array($matches) || count($matches) != 3) {
        throw new osapi_Exception("Error retrieving request key ({$ret['data']})");
      }
      return new OAuth_Token(urldecode($matches[1]), urldecode($matches[2]));
    } else {
      throw new osapi_Exception("Error requesting oauth request token, code " . $ret['http_code'] . ", message: " . $ret['data']);
    }
  }
  ...
  protected function requestRequestToken($callbackUrl) {
    $requestTokenRequest = OAuth_Request::from_consumer_and_token($this->consumerToken, NULL, "GET", $this->provider->requestTokenUrl, array());
    ...
    return osapi_IO::send($requestTokenRequest, 'GET', $this->provider->httpProvider);
  }
