...
class osapi_Request {
   ...
   * @return osapi_Request the generated request
   */
  public static function createRequest($method, $params) {
    ...
    if (! in_array(self::getService($method), $availableServices)) {
      throw new osapi_Exception("Invalid service: ".self::getService($method));
    }
    // Verify the method
    if ((self::getService($method) == 'cache' && self::getOperation($method) != 'invalidate') ||
      ...
      throw new osapi_Exception("Invalid method: ".self::getOperation($method));
    }
    if (self::getService($method) != 'cache' && self::getService($method) != 'system') {
	    // Verify base params
	    if (! isset($params['userId'])) {
	      throw new osapi_Exception("Invalid or missing userId");
	    }
	    ...
	    if (isset($params['groupId']) && ! in_array($params['groupId'], array('@self', '@all', '@friends', '@supportedMood'))) {
	      throw new osapi_Exception("Invalid groupId, allowed types are: @self, @all and @friends");
	    }
    }
