...
/**
 * The osapi_Logger class provides a logger for osapi library. You can
 * initialize it as follows:
 *
 * osapi_Logger::setLevel(osapi_Logger::INFO);
 * osapi_Logger::setAppender(new osapi_Logger_Appender_File("/tmp/log/osapi.log"));
 *
 * To make logs, you can call
 *
 * osapi_Logger::warn($yourMessage); or
 * osapi_Logger::info($yourObject);
 *
...
class osapi_Logger {
  ...
  public static function setLevel($newLevel) {
    if (!is_int($newLevel)) {
      throw new osapi_Logger_Exception($newLevel . " is not a number.");
    }

    if($newLevel > self::NONE || $newLevel < self::ALL) {
      throw new osapi_Logger_Exception("Level should be between ALL(0) " .
          "and NONE(6).");
    }
    self::$level = $newLevel;
  }
  ...
  static function log($level, $message) {
    if (!is_int($level)) {
      throw new osapi_Logger_Exception($level . " is not a number.");
    }
