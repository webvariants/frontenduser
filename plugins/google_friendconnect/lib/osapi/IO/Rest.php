 ...
 * @author Chris Chabot
 */
class osapi_IO_Rest extends osapi_IO {
  ...
  public static function sendBatch(Array $requests, osapi_Provider $provider, osapi_Auth $signer, $strictMode = false) {
    ...
    foreach ($requests as $request) {
      ...
      if (!$entry instanceof osapi_Model_Error) {
   ...
   */
  private static function executeRestRequest(osapi_Request $request, osapi_Provider $provider, osapi_Auth $signer) {
    $service = $request->getService($request->method);
    $operation = $request->getOperation($request->method);

    if (! isset(self::$urlTemplates[$service])) {
      throw new osapi_Exception("Invalid service: $service");
    }
    $urlTemplate = self::$urlTemplates[$service];

    if (! isset(self::$methodAliases[$operation])) {
      throw new osapi_Exception("Invalid method: ($service) $operation");
    }
    ...
    if (($response['http_code'] == '200' || $response['http_code'] == '201')  && !empty($response['data'])) {
      ...
      if ($ret['data'] == $response['data']) {
        ...
        throw new osapi_Exception("Error decoding server response: '" . $response['data'] . "'");
      }
    } else {
      $ret = new osapi_Model_Error($response['http_code'], isset($response['data']) ? $response['data'] : '');
    }
