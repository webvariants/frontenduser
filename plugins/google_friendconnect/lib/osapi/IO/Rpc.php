 ...
 * @author Chris Chabot
 */
class osapi_IO_Rpc extends osapi_IO {
  /**
  ...
  public static function sendBatch(Array $requests, osapi_Provider $provider, osapi_Auth $signer, $strictMode = false) {
    ...
    if ($ret['http_code'] == '200') {
      ...
      if ($result == $ret['data']) {
        throw new osapi_Exception("Error decoding response body:\n" . $ret['data']);
      }
      if (isset($result['code']) && $result['code'] == '401') {
        throw new osapi_Auth_Error("Authentication error: {$result['message']}");
      }
      if (is_array($result)) {
        ...
        foreach ($result as $entry) {
          ...
          if (isset($entry['error'])) {
            $entry['data'] = new osapi_Model_Error(isset($entry['error']['code']) ? $entry['error']['code'] : 500, isset($entry['error']['message']) ? $entry['error']['message'] : 'Unknown error occured');
            unset($entry['error']);
          } else {
            if (isset($entry['data']['entry'])) {
              if ($strictMode) {
                throw new osapi_Exception("Invalid RPC response body, collection key should be a 'list' and not 'entry'");
              } else {
      ...
      throw new osapi_Exception("Error sending RPC request: " . $ret['data']);
    }
  }
}
