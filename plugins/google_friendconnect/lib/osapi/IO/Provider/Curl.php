...
/**
 * The osapi_IO_Provider_Curl class is used to define the HTTP layer that will relay
 * your requests to the remote server, using the PHP curl library.
 *
 * @author Dan Holevoet
 */
class osapi_IO_Provider_Curl extends osapi_IO_Provider_Http {
  /**
  ...
  public function send($url, $method, $postBody = false, $headers = false, $ua = self::USER_AGENT) {
    ...
    osapi_Logger::info("HTTP Request");
    osapi_Logger::info($request);
    ...
    @curl_close($ch);
    if ($errno != CURLE_OK) {
      throw new osapi_Exception("HTTP Error: " . $error);
    }
    ...
    osapi_Logger::info("HTTP Response");
    osapi_Logger::info($response);
