...
/**
 * The osapi_Logger_Appender_File writes logs to console.
 *
 * @author Anash P. Oommen
 */
class osapi_Logger_Appender_File extends osapi_Logger_Appender {
  protected $logFile = "";
  ...
  public function appendMessage($level, $message) {
    ...
    if($level > osapi_Logger::NONE || $level < osapi_Logger::ALL) {
  ...
  private function createLock() {
    ...
    if (! is_dir($logDir)) {
      if (! @mkdir($logDir, 0755, true)) {
        ...
        if (! is_dir($logDir)) {
          throw new osapi_Storage_Exception("Could not create storage " .
              "directory: $logDir");
        }
      }
    }
