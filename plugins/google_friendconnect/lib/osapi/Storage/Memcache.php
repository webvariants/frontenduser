 ...
 * @author Chris Chabot
 */
class osapi_Storage_Memcache extends osapi_Storage {
  private $connection = false;

  public function __construct($host, $port) {
    if (! function_exists('memcache_connect')) {
      throw new osapi_Storage_Exception("Memcache functions not available");
    }
    if ($host == '' || $port == '') {
      throw new osapi_Storage_Exception("You need to supply a valid memcache host and port");
    }
    $this->host = $host;
    $this->port = $port;
  }
  ...
  private function connect() {
    if (! $this->connection = @memcache_pconnect($this->host, $this->port)) {
      throw new osapi_Storage_Exception("Couldn't connect to memcache server");
    }
  }
  ...
  public function set($key, $value) {
    ...
    if (@memcache_set($this->connection, $key, array('time' => time(),
      ...
      throw new osapi_Storage_Exception("Couldn't store data in cache");
    }
  }
