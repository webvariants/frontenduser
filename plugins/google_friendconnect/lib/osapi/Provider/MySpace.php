 ...
 * @author Chris Chabot
 */
class osapi_Provider_MySpace extends osapi_Provider {
  ...
  public function preRequestProcess(&$request, &$method, &$url, &$headers, osapi_Auth &$signer) {
    ...
    if ($signer instanceof osapi_Auth_OAuth_2Legged) {
  ...
  private function formatAppDataOut(osapi_Request &$request) {
    $data = new osapi_Model_AppData();
   ...
   * @param array $response
   */
  private function fixStatusLink(osapi_Request &$request, &$response) {
  ...
  private function formatAppDataIn(osapi_Request &$request, &$response) {
   ...
   * @param array $response
   */
  private function fixModelContainer(osapi_Request &$request, &$response) {
    $plural_rules = array(
      'groups'=>'group',
      'people'=>'person',
      'albums'=>'album',
