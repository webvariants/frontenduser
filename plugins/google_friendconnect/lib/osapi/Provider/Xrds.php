...
/**
 * The osapi_Provider_Xrds class attempts to auto-detect the OAuth and OpenSocial/PortableContacts
 ...
 * @author Chris Chabot
 */
class osapi_Provider_Xrds extends osapi_Provider {
  public $httpProvider;

  private $providerUrl;

  public function __construct($providerUrl, osapi_Storage $storage, osapi_IO_Provider_Http $httpProvider = null) {
    ...
    if (($xrds = $storage->get($this->providerUrl.":xrds", 24 * 60 * 60)) !== false) {
      ...
      if (empty($requestTokenUrl) || empty($authorizeUrl) || empty($accessTokenUrl)) {
        throw new osapi_Exception("Could not discover the required OAuth end-points");
      }
      ...
      if (empty($restEndpoint) && empty($rpcEndpoint) && empty($pocoUrl)) {
        throw new osapi_Exception("No supported social end-points found");
