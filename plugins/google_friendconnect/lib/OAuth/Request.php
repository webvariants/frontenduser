...
class OAuth_Request {/*{{{*/
  ...
  public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL) {/*{{{*/
    ...
    if ($parameters) {
      ...
      if (@substr($request_headers['Authorization'], 0, 6) == "OAuth ") {
        ...
        $req = new self($http_method, $http_url, $parameters);
      } else $req = new self($http_method, $http_url, $req_parameters);
    }
  ...
  public function get_signable_parameters() {/*{{{*/
    ...
    $keys = OAuth_Util::urlencode_rfc3986(array_keys($params));
    $values = OAuth_Util::urlencode_rfc3986(array_values($params));
  ...
  public function to_postdata() {/*{{{*/
    ...
    foreach ($this->parameters as $k => $v) {
      if (is_array($v)) {
        foreach ($v as $va) {
          $total[] = OAuth_Util::urlencode_rfc3986($k) . "[]=" . OAuth_Util::urlencode_rfc3986($va);
        }
      } else {
        $total[] = OAuth_Util::urlencode_rfc3986($k) . "=" . OAuth_Util::urlencode_rfc3986($v);
      }
    }
    $out = implode("&", $total);
    return $out;
  ...
  public function to_header() {/*{{{*/
    ...
    foreach ($this->parameters as $k => $v) {
      ...
      if (is_array($v)) throw new OAuth_Exception('Arrays not supported in headers');
      $out .= ',' . OAuth_Util::urlencode_rfc3986($k) . '="' . OAuth_Util::urlencode_rfc3986($v) . '"';
    }
    return $out;
  }/*}}}*/
  ...
  private static function split_header($header) {/*{{{*/
    ...
    while (preg_match($pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset) > 0) {
      ...
      $params[$header_name] = OAuth_Util::urldecode_rfc3986( $header_content );
