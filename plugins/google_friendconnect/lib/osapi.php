...
class osapi {
   ...
   *
   * @param osapi_Provider $provider provider to use (myspace, partuza, plaxo, xrds, etc)
   * @param osapi_Auth $signer signer to use (security token, 2 legged oauth or 3 legged oauth)
   */
  public function __construct(osapi_Provider $provider, osapi_Auth $signer) {
  ...
  public function newBatch() {
    return new osapi_Batch($this->provider, $this->signer, $this->strictMode);
  }
  ...
  public function makeRequest() {
    throw new osapi_Exception("makeRequest is not supported");
  }
