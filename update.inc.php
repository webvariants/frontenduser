...
try {
	...
	if (version_compare($oldVersion, '1.1', '<')) {
		$sql->query('ALTER TABLE ~wv16_users ADD COLUMN `confirmation_code` varchar(20) NOT NULL DEFAULT "" AFTER `was_activated`', null, '~');
	}

	if (version_compare($oldVersion, '1.2', '<')) {
		// helptext ergänzen
		$sql->query('ALTER TABLE ~wv16_attributes ADD COLUMN `helptext` varchar(1024) NOT NULL DEFAULT "" AFTER `title`', null, '~');
		$sql->query('ALTER TABLE ~wv16_attributes ADD COLUMN `hidden` tinyint(1) unsigned not null default "0" AFTER `default_value`', null, '~');
	}

	if (version_compare($oldVersion, '2.0', '<')) {
		// Spalten ergänzen
		$sql->query('ALTER TABLE ~wv16_users ADD COLUMN `activated` tinyint(1) unsigned NOT NULL DEFAULT "0" AFTER `deleted`', null, '~');
		$sql->query('ALTER TABLE ~wv16_users ADD COLUMN `confirmed` tinyint(1) unsigned NOT NULL DEFAULT "0" AFTER `activated`', null, '~');
		...
		$statement = $sql->prepare('UPDATE ~wv16_users SET activated = ?, confirmed = ? WHERE id = ?', '~');

		foreach ($data as $userID => $info) {
			$sql->query($statement, array($info['activated'], $info['confirmed'], $userID));
		}

		// Gruppenzugehörigkeiten entfernen
		$sql->query('DELETE FROM ~wv16_user_groups WHERE id IN (1,2,3)', null, '~');
		$sql->query('ALTER TABLE ~wv16_groups DROP COLUMN `internal`', null, '~');

		// wv16_rights entfernen
		$sql->query('DROP TABLE IF EXISTS ~wv16_rights', null, '~');
